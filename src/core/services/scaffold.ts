export type ScaffoldFile = {
	relativePath: string;
	content: string;
};

export type ScaffoldResult = {
	created: string[];
	skipped: string[];
};

/** Default workspace core manifest written to `.dakiya/dakiya.yaml`. */
export function buildManifestContent(workspaceName: string): string {
	return [
		`# Dakiya workspace core — metadata for this project`,
		`name: ${workspaceName}`,
		`description: "A local-first API workspace for ${workspaceName}."`,
		``,
		`versions:`,
		`  - v1`,
		``,
		`# Active environment (matches a file under environments/)`,
		`defaultEnv: local`,
		``,
	].join("\n");
}

export function buildLocalEnvContent(): string {
	return JSON.stringify({}, null, 2);
}

/** TypeScript type declarations for Dakiya scripts — enables IDE autocomplete. */
export function buildScriptTypesContent(): string {
	return [
		`// Auto-generated by \`dakiya init\` — gives you full IDE autocomplete in scripts.`,
		`// Do not edit manually; re-run \`dakiya init\` to regenerate.`,
		``,
		`declare module "dakiya/scripts" {`,
		`  /** Mutable view of the outgoing HTTP request. */`,
		`  export interface ReqApi {`,
		`    method: string;`,
		`    url: string;`,
		`    /** The parsed request body — cast to the shape you need. */`,
		`    body: unknown;`,
		`    getHeader(name: string): string | undefined;`,
		`    setHeader(name: string, value: string): void;`,
		`    removeHeader(name: string): void;`,
		`  }`,
		``,
		`  /** Read-only view of the HTTP response (post-script only). */`,
		`  export interface ResApi {`,
		`    readonly status: number;`,
		`    readonly statusText: string;`,
		`    /** Raw response body string. */`,
		`    readonly body: string;`,
		`    /** Parse the response body as JSON. */`,
		`    json(): unknown;`,
		`    getHeader(name: string): string | undefined;`,
		`    setHeader(name: string, value: string): void;`,
		`  }`,
		``,
		`  /** Environment variable bag — read, write, and persist values. */`,
		`  export interface EnvApi {`,
		`    get(name: string): string | undefined;`,
		`    /** @param options.persist — if true, the value is saved back to the .json env file. */`,
		`    set(name: string, value: string, options?: { persist?: boolean }): void;`,
		`    delete(name: string): void;`,
		`    /** Permanently commit variable changes to the active .json environment file on disk. */`,
		`    commit(name?: string): void;`,
		`  }`,
		``,
		`  /** Request metadata available in all scripts. */`,
		`  export interface ScriptMeta {`,
		`    /** Human-readable request name (e.g. "login"). */`,
		`    requestName: string;`,
		`    /** Path relative to collections root (e.g. "auth/login/post"). */`,
		`    requestPath: string;`,
		`    phase: "pre" | "post";`,
		`    /** Unique UUID generated per request send — useful for correlation. */`,
		`    traceId: string;`,
		`    /** Elapsed ms from send start. Only present in @post scripts. */`,
		`    durationMs?: number;`,
		`  }`,
		``,
		`  /** Context object passed to pre-request scripts. */`,
		`  export interface PreContext {`,
		`    req: ReqApi;`,
		`    env: EnvApi;`,
		`    meta: ScriptMeta & { phase: "pre" };`,
		`  }`,
		``,
		`  /** Context object passed to post-request scripts. */`,
		`  export interface PostContext {`,
		`    req: ReqApi;`,
		`    res: ResApi;`,
		`    env: EnvApi;`,
		`    meta: ScriptMeta & { phase: "post"; durationMs: number };`,
		`  }`,
		`}`,
		``,
	].join("\n");
}

/** tsconfig.json for the .dakiya/scripts/ folder to enable IntelliSense. */
export function buildScriptsTsConfigContent(): string {
	return JSON.stringify(
		{
			compilerOptions: {
				target: "ES2022",
				module: "ESNext",
				moduleResolution: "bundler",
				strict: true,
				paths: {
					"dakiya/scripts": ["./dakiya-env.d.ts"],
				},
			},
			include: ["collections/**/*.ts"],
			exclude: ["node_modules"],
		},
		null,
		2,
	);
}

/** Production-clean files for a fresh `.dakiya/` workspace (no sample requests). */
export function scaffoldFiles(workspaceName: string): ScaffoldFile[] {
	return [
		{
			relativePath: "dakiya.yaml",
			content: buildManifestContent(workspaceName),
		},
		{
			relativePath: "environments/local.json",
			content: buildLocalEnvContent(),
		},
		{
			relativePath: "dakiya-env.d.ts",
			content: buildScriptTypesContent(),
		},
		{
			relativePath: "tsconfig.json",
			content: buildScriptsTsConfigContent(),
		},
	];
}

